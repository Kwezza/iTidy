...
# Classic Branch Preparation — Shared Core Import

```text
Classic Branch Preparation — Shared Core Import

- Date: 2026-08-14
- Target branch: dev-classic
- Based on: v1 (3ea93d5 Correct version number from 2.0 to 1.0)
- Validated source tree:
    C:\Amiga\Programming\iTidy2
- Target tree:
    C:\Amiga\Programming\iTidy
- Shared folders imported:
    shared/image/ (complete)
    shared/icon/ (complete)
- Shared test files imported:
    src/tests/test_shared_image.c
    src/tests/test_shared_icon.c
    src/tests/test_shared_coloricon.c
    src/tests/test_shared_decode.c
    src/tests/amiga/icon_decode_test.c
    src/tests/host_stubs/exec/types.h
- Synthetic corpus imported:
    tests/icons/ (complete, including malformed streams)
- Real corpus imported:
    TestsIcons/test-icons/ (complete; DPaintV/AmosPro not copied)
- Amiga validator imported:
    src/tests/amiga/icon_decode_test.c
    built to Bin/Amiga/Tests/iTidyIconTest (68000, not committed)
- Byte/hash verification:
    Files copied: 93
    Byte-identical (SHA-256): 93
    Differences: 0
    Missing destination: 0
    Unexpected files: 0
- Makefile.tests created:
    test-image, test-icon, test-coloricon, test-decode,
    test-icon-amiga, test-all, clean-tests
    Host: gcc. Amiga: vc +aos68k -cpu=68000.
    Test-only platform stub: src/tests/stubs/platform/platform.h
    (Classic src/platform/platform.h enables DEBUG_MEMORY_TRACKING
    and must not be used by the shared-test harness.)
- Host test result:
    make -f Makefile.tests test-all — ALL PASSED
    (one expected SKIP: Bin/Amiga/iTidy2.info not present)
- Amiga validator build:
    make -f Makefile.tests test-icon-amiga — SUCCESS
    vc +aos68k -cpu=68000, no warnings from migrated shared source
- Existing Classic build:
    make — SUCCESS (Bin/Amiga/iTidy). Production Makefile unchanged.
    Known SDK header warnings 51/61 only.
- Production integration started: NO
- Migration commit:
    local checkpoint on dev-classic; not pushed
- Remaining prerequisite for Batch 5:
    Consume shared/image and shared/icon from Classic production
    Makefile/link. Do not start conversion UI until the shared
    modules compile in Classic.
- Notes:
    iTidy2 was not modified. dev-itidy2 was not merged.
    Shared decoder/image source was copied byte-for-byte.
...
Batch 4 validation CLI (iTidy2 tree): make test-icon-amiga ->
Bin/Amiga/iTidy2/iTidyIconTest
Classic imported CLI: make -f Makefile.tests test-icon-amiga ->
Bin/Amiga/Tests/iTidyIconTest
...
Classic branch preparation (2026-08-14): shared core, tests, corpora,
and docs imported onto dev-classic from v1. Host tests pass. Amiga
validator builds for 68000. Classic production `make` still succeeds.
Production integration has NOT started. Batch 5 may start on
dev-classic only.
```
