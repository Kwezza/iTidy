...
The AmigaOS 3.5/3.9 **ColorIcon** format is unusually well suited to iTidy’s goal. The colour imagery is not dependent on `icon.library` to make sense of it: after the conventional Amiga `.info` representation, a self-contained IFF `FORM ICON` is appended. Its core OS3.5/3.9 representation consists of a `FACE` chunk describing the common dimensions/aspect and one or two `IMAG` chunks containing chunky palette indices plus an RGB palette. The first `IMAG` is the normal image; the second, when present, is the selected image. Current AROS source independently implements this format in `workbench/libs/icon/diskobj35io.c`. fileciteturn22file0L2-L2 fileciteturn23file0L2-L2

**The important conclusion for iTidy is that decoding genuine OS3.5/3.9 `IMAG` ColorIcons needs neither V44 `icon.library`, datatypes, zlib nor PNG support.** The compression used for `IMAG` pixels and palettes is a small PackBits-like run-length scheme. It is simple enough for a few dozen lines of C89. AROS includes zlib in this source file only because its modern implementation also understands an `ARGB` extension; zlib is not used for `IMAG`. fileciteturn22file0L2-L2

There is one especially important implementation detail that is easy to get wrong: compressed image data is a **continuous bitstream**. RLE control codes are 8 bits, but pixel values use `Depth` bits. With `Depth < 8`, a control code following pixel data can therefore begin in the middle of a byte. A decoder which treats compressed input as ordinary byte-aligned PackBits will only work reliably at depth 8. AROS's `Decode35()` explicitly maintains a bit reservoir to handle this. fileciteturn22file0L2-L2
...
The V44 API documentation independently confirms the logical model exposed by the original library: palette-mapped images are one byte per pixel after decoding, have 1–256 colours, dimensions of 1–256 pixels in either axis, separate normal/alternate images, an optional transparent palette index, and access to the palette as originally created. It also distinguishes original second-image data from an alternate image which `icon.library` may synthesise itself. citeturn10view0
...
The current official AROS repository is the active upstream source tree, and AROS's own developer documentation identifies GitHub as its central repository. citeturn8search0turn8search1
...
The **fixed serialized `DiskObject` header** is 78 bytes. AROS's `DiskObjectDesc` and `GadgetDesc` explicitly describe the on-disk fields; pointer-valued members are represented in the file as four-byte presence values, not host pointers. After that fixed 78-byte region come variable-size classic icon structures in a defined order. fileciteturn24file0L2-L2
...
But **byte 78 is not normally the start of `FORM ICON`**. AROS proceeds from that fixed structure through DrawerData, conventional planar images, strings, ToolTypes and the newer DrawerData fields; only then does `ProcessIcon35` invoke `ReadIcon35()` at the stream's current position. `ReadIcon35()` explicitly records that current position as where its “extra data” begins. fileciteturn24file0L2-L2 fileciteturn25file0L2-L2 fileciteturn23file0L2-L2
...
This order follows `IconDesc` in `diskobjio.c`: `ProcessOldDrawerData`, `ProcessGadgetRender`, `ProcessSelectRender`, `ProcessDefaultTool`, `ProcessToolTypes`, `ProcessToolWindow`, `ProcessNewDrawerData`, then `ProcessIcon35`. fileciteturn24file0L2-L2
...
which corresponds to 16-pixel word alignment per row per bitplane. AROS's `ReadImage()` uses exactly this calculation. fileciteturn25file0L2-L2

Strings are stored as a big-endian 32-bit length followed immediately by that many string bytes. ToolTypes begin with a 32-bit value representing the serialized four-byte pointer-table size; AROS derives the number of strings as `(count >> 2) - 1`, then reads each as another length-prefixed string. fileciteturn25file0L2-L2

For a drawer using the supported Workbench disk revision, AROS reads an additional `ULONG dd_Flags` followed by `UWORD dd_ViewModes`, i.e. six bytes, before it reaches `ProcessIcon35`. fileciteturn25file0L2-L2
...
All multi-byte quantities in these serialized structures should be interpreted in Motorola/big-endian order. This is also how AROS explicitly handles fields such as the `IMAG` length bytes and later ARGB sizes, and `icontool` uses `>H` and `>I` throughout. fileciteturn22file0L2-L2 citeturn13view1
...
is inappropriate on a 68000 if `p` may be odd. This is an inference from the AROS serialization path rather than an explicit AmigaOS format statement. fileciteturn25file0L2-L2 fileciteturn26file0L2-L2
...
The current AROS ColorIcon reader recognises `FACE`, `IMAG`, `ARGB`, and lower-case `png ` chunks inside `FORM ICON`. The original palette-mapped ColorIcon path is `FACE` plus `IMAG`; the latter two are later true-colour/image extensions and are not required to decode an OS3.5/3.9 palette ColorIcon. fileciteturn22file0L2-L2 fileciteturn23file0L2-L2

```mermaid
flowchart TD
    A[".info file"] --> B["Classic DiskObject serialisation"]
    B --> C["FORM ICON"]
    C --> D["FACE — dimensions, flags, aspect, palette maximum"]
    C --> E["IMAG #1 — normal palette-mapped image"]
    C --> F["IMAG #2 — selected image, optional"]
    C --> G["ARGB — later true-colour extension"]
    C --> H["png  — later extension"]
    C --> I["Unknown chunks — skip safely"]
```

For the **phase-one iTidy decoder**, I would define “ColorIcon successfully decoded” as:
...
The V44 `IconControlA()` contract confirms that palette-mapped icon image data is exposed as one `UBYTE` per pixel, with exactly `width × height` image bytes after decoding, and palettes of 1–256 entries. citeturn10view0
...
and its writer stores actual width and height minus one. fileciteturn22file0L2-L2 fileciteturn26file0L2-L2
...
The V44 API independently constrains palette-mapped icon width and height to 1–256. citeturn10view0

AROS defines `ICON35F_FRAMELESS` as bit 0. Unknown higher bits should be ignored for rendering conversion but retained as metadata if iTidy later rewrites ColorIcons. fileciteturn27file0L2-L2
...
This definition and interpretation are directly implemented by AROS `ReadImage35()`, and `icontool` independently parses the same ten fields. fileciteturn22file0L2-L2 citeturn13view1
...
AROS implements those exact `+1` calculations. fileciteturn22file0L2-L2

### Raw image representation

When `ImageFormat == 0`, the image is **not packed according to `Depth`**. AROS simply copies `width × height` bytes from the stream into the chunky pixel array. In other words, even a two-colour uncompressed image uses one byte per pixel. fileciteturn22file0L2-L2
...
AROS currently performs the copy without first proving that the declared raw image area is that large, so this is a worthwhile hardening improvement rather than behaviour to imitate. fileciteturn22file0L2-L2
...
Each component is eight bits. fileciteturn22file0L2-L2
...
The V44 API describes the retrieved palette as the palette corresponding to the colours in use when the icon was created, with a palette size up to 256 entries. citeturn10view0
...
If the second `IMAG` has no palette of its own, AROS assigns it the first image's palette and pen count. This palette inheritance is important and should be reproduced by iTidy. fileciteturn23file0L2-L2
...
Do **not** fabricate Workbench colours when a required source palette is genuinely unavailable. Current `icontool` does fabricate default WB palettes in such a situation; that is convenient for image export but is not a faithful reconstruction of the ColorIcon data. citeturn14view0

When there is no stored second `IMAG`, iTidy should report “selected image not stored”. Original V44 `icon.library` could generate an alternate image itself, and `ICONCTRLA_HasRealImage2` exists specifically to distinguish a genuinely stored second image from one generated by the library. citeturn10view0
...
The V44 public API uses exactly this model: a palette index identifies the transparent colour, while `-1` represents an opaque image. It also requires the transparent index to be within the palette. citeturn10view0

There is a notable **AROS implementation anomaly** here. Its internal header clearly defines bit 0 as `HASTRANSPARENTCOLOR`, but current `ReadIcon35()` copies `ic.TransparentColor` into the native structure without testing the flag; current `WriteImage35()` also appears internally inconsistent, setting the flag for a non-negative transparent index and then writing `255` when that flag is set. I would treat these current-source details as bugs or compatibility artefacts, not as the semantic specification. fileciteturn23file0L2-L2 fileciteturn26file0L2-L2

For iTidy, the flag plus V44 API semantics are the safer rule.

### `ARGB` and `png `

Current AROS also accepts `ARGB` and `png ` chunk IDs within `FORM ICON`. This is useful for recognising a later icon without misunderstanding it as `IMAG`, but neither needs implementation for the OS3.5/3.9 palette decoder. fileciteturn22file0L2-L2 fileciteturn23file0L2-L2
...
Dimensions come from `FACE`; expected decompressed length is `width × height × 4`. fileciteturn22file0L2-L2

That path invokes zlib `uncompress()`. **The `IMAG` path does not.** Therefore iTidy can simply recognise and skip `ARGB` without bringing any zlib dependency into a WB2.x–3.1 build. fileciteturn22file0L2-L2

Current `icontool` disagrees materially about `ARGB`: its `parse_argb_chunk()` reads two big-endian 16-bit dimensions followed by uncompressed ARGB bytes. citeturn13view2 This divergence is another reason not to let ARGB work dilute the ColorIcon phase. It needs separate research before implementation.

## Compression, palette reconstruction and worked bytes

### The exact `IMAG` compression

AROS comments that `Decode35()` was based on Dirk Stöcker's `ModifyIcon`. Its decoder takes:
...
and uses the same routine for both pixels and palettes. For pixels, `bits = Depth`; for a compressed palette, `bits = 8` and the expected number of entries is `3 × num_colours`. fileciteturn22file0L2-L2
...
This follows AROS's exact branches: `<=127` means literal; `>128` means repeat; exactly `128` continues without consuming an entry. fileciteturn22file0L2-L2
...
With other depths, the literal/repeated values need not end on a byte boundary. The **next control byte is read from the next eight bits of that same stream**, wherever those bits begin. AROS's `bitbuf/numbits` logic is specifically written that way. fileciteturn22file0L2-L2
...
The V44 API distinguishes that source palette from the later remapping of palette-mapped icons to a screen. `IconControlA()` says the retrieved palette has the number of entries used when the icon was created, while global precision/screen controls govern remapping for display. citeturn10view0
...
then interpret the result as those same RGB triples. AROS does exactly this. fileciteturn22file0L2-L2
...
Although `Depth` is 1, the uncompressed image still contains **four full bytes**, one for each pixel, matching AROS's raw path. fileciteturn22file0L2-L2
...
AROS is particularly valuable here because it does not merely expose the V44 API; it has to independently interpret the file representation. The official AROS project describes its GitHub repository as its central source tree. citeturn8search0turn8search1

### AROS source map

| Requirement | AROS file/function | What it establishes |
|---|---|---|
| fixed classic serialization | `diskobjio.c` — `GadgetDesc`, `DiskObjectDesc`, `ImageDesc` | exact serialized field ordering |
| variable classic ordering | `diskobjio.c` — `IconDesc` | exact point at which ColorIcon parsing starts |
| classic planar image size | `diskobjio.c` — `ReadImage()` | planar byte calculation |
| `FACE` / `IMAG` structs | `diskobj35io.c` — `FileFaceChunk`, `FileImageChunk` | exact field sizes/order |
| flags | `icon_intern.h` | transparent, palette, frameless bits |
| RLE decoder | `diskobj35io.c` — `Decode35()` | exact control and bitstream semantics |
| palette/pixels | `diskobj35io.c` — `ReadImage35()` | raw versus packed image; RGB palette |
| first/second image | `diskobj35io.c` — `ReadIcon35()` | normal/select ordering |
| palette inheritance | `diskobj35io.c` — `ReadIcon35()` | selected image can share first palette |
...
- aspect information;
- frameless property;
- an alternate image may be generated when none is physically stored. citeturn10view0
...
The V44 write API also explicitly distinguishes preserving/dropping traditional planar icon imagery from preserving/dropping the chunky palette-mapped imagery. That corroborates the architectural model of a traditional icon plus an additional colour representation. citeturn10view1
...
`parse_imag_chunk()` agrees with AROS on the ten-byte header, byte order, image-first/palette-second order, colour-count `+1`, depth, and size `+1`. citeturn13view1

`parse_glowicons()` agrees on `FORM ICON`, `FACE`, ordered image chunks and IFF word padding. citeturn14view1
...
The `0x80` distinction can be seen directly in `icontool`'s RLE routines versus AROS's `Decode35()`. citeturn13view0turn14view1 fileciteturn22file0L2-L2

The selected-palette difference is similarly explicit: AROS assigns `img1pal` to image two if no second palette was loaded, whereas `icontool` substitutes one of its built-in palettes when palette data is unavailable. fileciteturn23file0L2-L2 citeturn14view0

The ARGB difference is sufficiently large that I would consider that format **unresolved for a future separate study**, not part of ColorIcon implementation. Current `icontool` labels it an “OS4 true-color format”, while current AROS has a different compressed representation. citeturn13view2 fileciteturn22file0L2-L2
...
The V44 dimensions cap the ColorIcon face at 256×256, and the palette at 256 entries. citeturn10view0
...
| RGBA expansion of one max image | 262,144 B — best avoided |

The `IMAG` encoded-size fields themselves are 16-bit “size minus one” values, so they can represent 1–65,536 encoded bytes. A decoder operating directly on the original `.info` buffer does not need a second copy of those encoded streams. fileciteturn22file0L2-L2
...
For the much more typical 46×46 OS3.5-style artwork, the index buffer is only 2,116 bytes. Contemporary ColorIcon sets were not actually limited to that size; for example an Aminet ColorIcon game collection explicitly uses 120×96 images. citeturn16search1
...
The `ci_rle_nop_1x1.info` fixture is particularly important: it will distinguish an AROS-compatible decoder from the current `icontool` behaviour at control value `0x80`. fileciteturn22file0L2-L2 citeturn13view0

### Real-world corpus

After synthetic tests, use freely obtainable period ColorIcon archives as integration tests.

`GameIcons5.lha` is unusually useful because its author explicitly states that all icons are in ColorIcon format, all are 120×96, each icon has two pictures, and most were made with 256 colours. The archive lists many actual `.info` members, including `Icons/LetterDrawers/small/a/Amegas.info`. citeturn16search1

`Lucy35Icons.lha` is another strong regression corpus because its author explicitly says the package contains old NewIcons converted to OS3.5 icons, with more than 60 game icons and a visible list of `.info` members. citeturn16search3

For smaller artwork, `Drw-Icn_OS35.lha` contains `Drw-Icn_blue.info` and explicitly requires OS3.5. citeturn16search5
...
The reason to reject RLE overshoots rather than silently truncate is particularly important. AROS's decoder stops when the requested output count is reached; that is fine in a trusted system library, but a converter processing arbitrary files gains nothing by accepting contradictory run lengths. fileciteturn22file0L2-L2
