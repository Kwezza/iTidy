...
gadget hint text (tooltips), list item labels, and window titles. Changes were
cross-referenced against the working notes in `docs/manual/working_notes/`.
...
- **65 hints updated** (existing tooltips reworded for clarity, including 9 follow-up tweaks)
- **11 list item labels updated** (chooser/cycle dropdown text improved)
- **2 window titles corrected**
- **6 gadget labels/names tightened** (pass 3 — visible text on buttons, checkboxes, chooser items, menu items)
- **116 total changes**
...
- `deficons_creation_window.md`: Max Colours option table updated to use British spelling
  ("colours" not "colors") and reordered to match the actual .reb chooser item order
  (GlowIcons palette moved to last position).

---

## Window Title Changes

| Window IDENT | Before | After |
|---|---|---|
| `main_progress_window` | iTidy - Status | **iTidy - Progress** |
| `restore_default_tool` | iTidy - Restore Default Tool | **iTidy - Restore Default Tools** |
...
| 2 | 8 | **8 colours** |
| 3 | 16 | **16 colours** |
| 4 | 32 | **GlowIcons palette (29 colours)** |
| 5 | 64 | **32 colours** |
| 6 | 128 | **64 colours** |
| 7 | 256 | **128 colours** |
| 8 | Ultra (256 + Detail Boost) | **256 colours (full)** |
| 9 | GlowIcons palette (29 colours) | **Ultra (256 + detail boost)** |

### dco_list_textgen_filter (`dco_list_textgen_filter`)

| # | Before | After |
|---|--------|-------|
| 1 | All | All |
| 2 | Custom | **Custom Only** |
| 3 | Missing | **Missing Only** |
...
---
## Follow-up Hint Refinements (Pass 2)

These 9 hints were refined during a second review pass to exactly match the working notes wording.

#### Strip Newicons Borders (Permanent) (Checkbox) [UPDATED]
*IDENT:* `adv_cb_strip_newicon_borders`

**Before:** Permanently strips the black border from NewIcons. Requires icon.library v44 or later. Enable backups before using.

**After:** Permanently strips the black border from NewIcons. Requires icon.library v44 or later. Enable backups before using this option.

#### Change Default Tool... (Button) [UPDATED]
*IDENT:* `dfs_btn_change_default_tools`

**Before:** Opens a file requester to assign a new default tool to the selected type. Changes are saved to disk immediately.

**After:** Opens a file requester to assign a new default tool to the selected type. Changes are saved to disk immediately and are not affected by Cancel.

#### Cancel (Button, deficons_setup) [UPDATED]
*IDENT:* `dfs_btn_cancel`

**Before:** Closes the window without saving type selection changes. Note: default tool changes are already saved to disk.

**After:** Closes the window without saving type selection changes. Note: default tool changes made via "Change Default Tool..." are already saved to disk.

#### GetFile (default_tool_update) [UPDATED]
*IDENT:* `dtu_ch_new_tool`

**Before:** Select the new default tool program. Leave empty to remove the tool association from the selected icon(s).

**After:** Select the new default tool program. Leave empty to remove the tool association from the selected icon(s). Cannot select .info files.

#### Skip Icon Creation Inside WHDLoad Folders (Checkbox) [UPDATED]
*IDENT:* `dco_cb_skip_whdload_folders`

**Before:** When enabled, files inside WHDLoad game folders are skipped during icon creation. The folder icon itself is still created.

**After:** When enabled, files inside WHDLoad game folders are skipped during icon creation. The WHDLoad folder icon itself is still created.

#### Restore Run (Button) [UPDATED]
*IDENT:* `rb_btn_restore`

**Before:** Restores icon positions and window layouts from the selected backup run. Choose with or without window positions. Requires LhA.

**After:** Restores icon positions and window layouts from the selected backup run. Choose to restore with or without window positions. Requires LhA in C:.

#### Backup Runs ListBrowser [UPDATED]
*IDENT:* `rb_lb_backups`

**Before:** Shows all iTidy backup runs. Click a run to view its details below. Double-click to view the folder structure.

**After:** Shows all iTidy backup runs. Click a run to view its details below. Double-click to view the folder structure of the run.

#### Tool Changes ListBrowser [UPDATED]
*IDENT:* `rdt_lb_changes`

**Before:** Shows the tool changes in the selected session: the original tool, the replacement, and how many icons were affected.

**After:** Shows the tool changes recorded in the selected session: the original tool, the replacement, and how many icons were affected.

#### Scan (Button) [UPDATED]
*IDENT:* `dta_btn_scan`

**Before:** Starts a recursive scan of the selected folder to find all default tools referenced by icons. Opens a progress window.

**After:** Starts a recursive scan of the selected folder to find all default tools referenced by icons. Opens a progress window during scanning.

---

## Gadget Label Tightening (Pass 3)

These 6 changes shorten or correct visible gadget text (button names, checkbox names, chooser items, menu items) identified by cross-referencing all working notes against the .reb file.

#### Cancel -> Close (Button, restore_backups) [UPDATED]
*IDENT:* `rb_btn_close`
*Field:* NAME
...
**Reason:** No pending changes to cancel in this window -- it just closes. All other comparable windows (folder_view, tool_cache, restore_default_tool) already use "Close".

#### Back Up Layouts Before Changes -> Back Up Layout Before Changes (Menu item) [UPDATED]
*Window:* `main_window` (main_menu)
*Field:* ITEMNAME

**Before:** Back Up Layouts Before Changes

**After:** Back Up Layout Before Changes

**Reason:** The checkbox label (IDENT `main_cb_backup_icons`) uses singular "Layout". The menu toggle should match.

#### Classic (1) -> Classic (1.0) (Chooser item) [UPDATED]
...
**Reason:** All other aspect options use one decimal place: Tall (0.75), Square (1.0), Compact (1.3), Wide (2.0). This makes the list consistent.

#### Skip Icon Creation Inside WHDLoad Folders -> Skip Files In WHDLoad Folders (Checkbox) [UPDATED]
*IDENT:* `dco_cb_skip_whdload_folders`
*Field:* NAME

**Before:** Skip Icon Creation Inside WHDLoad Folders

**After:** Skip Files In WHDLoad Folders

**Reason:** 31% shorter (30 vs 45 chars). "Icon creation" context is already clear from the Create tab. The hint provides the full explanation.

#### Smart (Create Folder If It Has Icons Inside) -> Smart (When Folder Has Icons) (Chooser item) [UPDATED]
*IDENT:* `dco_list_folder_icon_mode`
*Field:* LISTITEM

**Before:** Smart (Create Folder If It Has Icons Inside)

**After:** Smart (When Folder Has Icons)

**Reason:** 36% shorter (30 vs 44 chars). In the "Folder Icons:" chooser the action (creating a folder icon) is already implied by context.

#### Upscale Small Images To Icon Size -> Upscale Small Images (Checkbox) [UPDATED]
*IDENT:* `dco_cb_upscale_thumbnails`
*Field:* NAME

**Before:** Upscale Small Images To Icon Size

**After:** Upscale Small Images

**Reason:** "To Icon Size" is implicit -- this checkbox sits on the Rendering tab next to the Preview Size chooser. The hint already explains the full behaviour.
