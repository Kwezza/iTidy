...
- **Smart Filtering** - Category-based filtering, system path exclusion, folder icon modes
- **Statistics Tracking** - Reports icons created by category
- **Creation Logging** - Optional log file for tracking created icons
- **Requirements**: DefIcons.library must be running and templates must be installed in `ENVARC:Sys/def_*.info`

**Icon Creation Modes:**
- **Smart Mode** (default): Create folder icons only when they have visible contents
- **Always Mode**: Create icons for all folders
- **Never Mode**: Skip folder icon creation
- **Category Filtering**: Disable specific file type categories (e.g., skip music, images)
- **System Path Exclusion**: Skip system assigns like SYS:, C:, DEVS:, LIBS:
...
### Main Processing Path: User Clicks "Apply"

1. **Event Handler** (`src/GUI/main_window.c`)
   - Captures `IDCMP_GADGETUP` event from Apply button
...
- **Memory**: At least 2MB free RAM (ReAction + icon processing)
...
## se at your own risk.** The author accepts no responsibility for data loss, corruption, or other issues.
...
3. **Choose Options**:
   ...
   - (Optional) Enable "Create new icons" to automatically create icons for iconless files using DefIcons
...
- iTidy v2 requires at least 2MB RAM (ReAction + icon processing)
- Try processing smaller directory trees
- Disable recursive processing for very large collections
- For systems with less RAM, use iTidy v1.x (GadTools version)
...
**v2.0** - Current Release (ReAction GUI + DefIcons)
- **ReAction GUI** - Complete rewrite using Workbench 3.2 built-in ReAction classes
  - Modern BOOPSI-based interface with automatic layout
  - No external dependencies (ReAction built into OS 3.2)
  - Enhanced user experience with advanced gadgets
- **DefIcons integration** for automatic icon creation
  - ARexx communication with DefIcons.library
  - Template-based icon assignment with hierarchy walking
  - Category filtering and system path exclusion
  - Smart/Always/Never folder icon modes
  - Statistics tracking and creation logging
- All v1.x features retained and enhanced
- **Requires**: Workbench 3.2+, 68000+, 2MB RAM
...
[ ] Use `FreeVec()` (not `FreeDosObject()`) for cleanup
- [ ] Buffer at least 512 bytes (preferably 1024) for deep paths
...
*This document was last updated: January 20, 2026*
