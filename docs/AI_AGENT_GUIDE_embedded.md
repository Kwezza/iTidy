...
static VOID sync_window_strings(struct TemplateWindowData *data)
{
    ...
    /* Repeat for any other STRING_KIND gadgets in this window */
}
```

Then in your **OK/Apply** button handler:

```c
case GID_OK_BUTTON:
    sync_window_strings(data);      /* Pull latest edits from string gadgets */
    perform_action_using_strings(data);
    break;
```

AI agents must adopt this pattern when:
- A window has STRING_KIND gadgets, **and**
- The user can confirm/execute via an OK/Apply button.

---

### 📚 Safe Dynamic ListView Updates

If you modify the contents of a ListView while the window is open, you must:

1. Temporarily detach the list from the gadget.
2. Mutate the Exec list (add/remove nodes, update text).
3. Reattach the updated list.

**Correct sequence:**

```c
static VOID refresh_listview_contents(struct TemplateWindowData *data)
{
    struct Gadget *listview = data->main_listview;
    struct Window *win      = data->window;
    struct List   *list     = &data->items_list;

    if (win == NULL || listview == NULL)
        return;

    /* 1) Detach list from gadget */
    GT_SetGadgetAttrs(listview, win, NULL,
                      GTLV_Labels, ~0UL,
                      TAG_END);

    /* 2) Modify the Exec list safely here */
    rebuild_items_list(list);

    /* 3) Reattach updated list */
    GT_SetGadgetAttrs(listview, win, NULL,
...
> ❗ Do **not** manipulate the `struct List` used by a ListView while it is still attached via `GTLV_Labels`. Detach–modify–reattach is the only supported pattern.

---

### 🍽️ NewLook Menu Handling Rules (OS3.0/3.1)

For Workbench 3.0/3.1, iTidy uses the **NewLook menu template**. AI agents must follow these rules:

- Menu IDs live in `nm_UserData` (or `UserData` via `GTMENUITEM_USERDATA`).
- On `IDCMP_MENUPICK`:
  - Treat `msg->Code` as an **encoded selection**, not a direct ID.
  - Use this pattern:
    - `ItemAddress(menu_strip, code)` → `GTMENUITEM_USERDATA(item)` → app-level ID.
    - Follow `item->NextSelect` until `MENUNULL` to support multiple picks.
- `CreateMenus()` → `LayoutMenus(..., GTMN_NewLookMenus, TRUE, ...)` → `SetMenuStrip(window, menu_strip)` must be used in that order.

**Do NOT use MENUVERIFY**

iTidy does **not** use `IDCMP_MENUVERIFY` or `MENUVERIFY` messages. AI agents must **not**:

- Add `IDCMP_MENUVERIFY` to the IDCMP mask.
- Implement or rely on `MENUVERIFY` behaviour.

Using `MENUVERIFY` incorrectly can deadlock menus if you open requesters before replying to the verify message. For safety and simplicity, iTidy only reacts to **normal** `IDCMP_MENUPICK` events using the NewLook menu template.

---

### 🖥️ Workbench vs CLI: Output Policy

Many Amiga examples use `printf()` / `Printf()` for debug output. For iTidy:

- When running as a **Workbench-launched** GUI tool:
  - There is **no guaranteed console** attached.
  - Uncontrolled `printf()` output can block or crash if no console exists.
  - AI agents must **not** add new `printf()` calls in GUI code unless:
    - A console window is explicitly opened, or
    - Output is clearly routed to a log file or log window.

- When running from a **CLI shell**:
  - `printf()`/`Printf()` are acceptable for debug builds.
  - Release builds should still prefer GUI requesters or log files for user-facing errors.

Always terminate the program by returning from `main()` (or using `exit()`), **never** by calling `Exit()` from Exec.
