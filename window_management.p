...
int FormatIconsAndWindow(char *folder) {
    ...
    if (lock) {
        ...
        __builtin_printf("Failed to lock the directory: %s\n", folder);
        return 1;
    }
    return 0;
}
...
void resizeFolderToContents(char *dirPath, IconArray *iconArray) {
    ...
    for (i = 0; i < iconArray->size; i++) {
        maxWidth = ((maxWidth) > (iconArray->array[i].icon_x + iconArray->array[i].icon_max_width) ? (maxWidth) : (iconArray->array[i].icon_x + iconArray->array[i].icon_max_width));
        maxHeight = ((maxHeight) > (iconArray->array[i].icon_y + iconArray->array[i].icon_max_height) ? (maxHeight) : (iconArray->array[i].icon_y + iconArray->array[i].icon_max_height));
    }
...
int InitializeWindow() {
    ...
    if (!screen) {
        __builtin_printf("Failed to lock Workbench screen.\n");
        return 0;
    }
    ...
    window = OpenWindowTags(0L,
                            ((((ULONG)(1UL<<31)) + 99) + 0x01), windowLeft,
                            ((((ULONG)(1UL<<31)) + 99) + 0x02), windowTop,
                            ((((ULONG)(1UL<<31)) + 99) + 0x03), windowWidth,
                            ((((ULONG)(1UL<<31)) + 99) + 0x04), windowHeight,
                            ((((ULONG)(1UL<<31)) + 99) + 0x08), 0x00000100,
                            ((((ULONG)(1UL<<31)) + 99) + 0x0D), screen,
                            (0UL));
    if (!window) {
        __builtin_printf("Failed to open window.\n");
        UnlockPubScreen("Workbench", screen);
        return 0;
    }
    ...
    if (!rastPort) {
        __builtin_printf("RastPort failed to create.\n");
    }
    font = screen->RastPort.Font;
    if (font) {
        SetFont(rastPort, font);
    } else {
        __builtin_printf("Failed to get default font from screen.\n");
    }
    return 1;
}
...
void repoistionWindow(char *dirPath, int winWidth, int winHeight) {
    ...
    winWidth += prefsIControl.currentBarWidth + prefsIControl.currentLeftBarWidth + (4 * 2);
    if (prefsWorkbench.disableVolumeGauge && IsRootDirectorySimple(dirPath))
        winWidth += prefsIControl.currentCGaugeWidth;
    winHeight += prefsIControl.currentWindowBarHeight + prefsIControl.currentBarHeight + (4 * 2);
