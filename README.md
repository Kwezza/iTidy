...
- `-dontResize` Do not resize and center the Workbench window to fit after arranging icons.
...
- `-skipWHD` By default, the program skips rearranging icons in WHDLoad folders to preserve the original author's layout. This option forces the program to rearrange the icons even if a .slave file is in the folder. Without this option, the folder's window will only be resized and centered.
