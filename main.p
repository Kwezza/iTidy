...
int main(int argc, char **argv)
{
    ...
    __builtin_printf("IControl prefs:  border width: %d, border height: %d\n", prefsIControl.currentBarWidth, prefsIControl.currentBarHeight);
    __builtin_printf("IControl prefs:  title height: %d, Window height: %d\n", prefsIControl.currentTitleBarHeight, prefsIControl.currentWindowBarHeight);
    __builtin_printf("IControl prefs:  volume guage width: %d\n", prefsIControl.currentCGaugeWidth);

    InitializeWindow();
    ProcessDirectory(argv[1], 0);
    CleanupWindow();
    return 0;
}
...
void CalculateTextExtent(const char *text, struct TextExtent *textExtent)
{
    ...
    ULONG textLength = __builtin_strlen(text);

    if (!rastPort)
    {
        __builtin_printf("RastPort is not initialized.\n");
        return;
    }
...
int saveIconsPositionsToDisk(IconArray *iconArray)
{
    ...
    for (i = 0; i < iconArraySize; i++)
    {
        ...
        if (diskObject)
        {
            ...
            {
                fprintf((&__iob[2]), "Error: Failed to save icon position for file: %s\n", fileNameNoInfo);
            }
        ...
        {
            fprintf((&__iob[2]), "Error: Failed to get DiskObject for file: %s\n", currentIcon->icon_full_path);
        }
    }
...
void SaveFolderSettings(const char *folderPath, folderWindowSize *newFolderInfo)
{
    ...
    __builtin_strcpy(diskInfoPath,folderPath);

    // Check if folder path ends with a colon or slash
    folderPathLen = __builtin_strlen(diskInfoPath);
...
int strncasecmp_custom(const char *s1, const char *s2, size_t n)
{
    while (n-- > 0 && *s1 != '\0' && *s2 != '\0')
    {
        char c1 = ((__ctype[((unsigned char)*s1)+1] & 1) ? (((unsigned char)*s1)+('a'-'A')) : ((unsigned char)*s1));
        char c2 = ((__ctype[((unsigned char)*s2)+1] & 1) ? (((unsigned char)*s2)+('a'-'A')) : ((unsigned char)*s2));
...
void removeInfoExtension(const char *input, char *output)
{
    ...
    size_t inputLen = __builtin_strlen(input);
    size_t extLen = __builtin_strlen(extension);
    ...
    else
    {
        ...
        __builtin_strcpy(output,input);
    }
} /* removeInfoExtension */

BOOL checkIconFrame(const char *iconName)
{
    struct DiskObject *icon = 0L;
    LONG frameStatus;
    LONG errorCode;
    struct Library *IconBase = 0L;
    const char *extension = ".info";
    size_t len = __builtin_strlen(iconName);
    size_t ext_len = __builtin_strlen(extension);

    // Calculate the length of the new icon name without the ".info" extension if present
    size_t new_len = (len >= ext_len && __builtin_strcmp(iconName + len - ext_len,extension) == 0) ? len - ext_len : len;
    ...
    if (!IconBase)
    {
        __builtin_printf("Failed to open icon.library\n");
        free(newIconName);
        return 1; // Assume it has a frame if library can't be opened
    }

    // Load the icon using the new name without the ".info" extension
    icon = GetIconTags(newIconName, (0UL));
    if (!icon)
    {
        __builtin_printf("Failed to load icon for border checks: %s\n", newIconName);
        CloseLibrary(IconBase);
        free(newIconName);
        return 1; // Assume it has a frame if icon can't be loaded
    }

    // Use IconControl to check the frame status of the icon
    if (IconControl(icon,
                    ((((ULONG)(1UL<<31))+0x9000)+33), &frameStatus,
                    ((((ULONG)(1UL<<31))+0x9000)+1), &errorCode,
                    (0UL)) == 1)
    {
    ...
    {
        __builtin_printf("Failed to retrieve frame information; error code: %ld\n", errorCode);
        PrintFault(errorCode, 0L);
    }
    ...
    CloseLibrary(IconBase);
    free(newIconName);
    return 1; // Default to having a frame if there's an error
}
