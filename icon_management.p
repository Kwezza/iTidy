...
void FreeIconArray(IconArray *iconArray)
{
    ...
    if (iconArray != 0L)
    {
        if (iconArray->array != 0L)
        {
            for (i = 0; i < iconArray->size; i++)
            {
                if (iconArray->array[i].icon_text != 0L)
                {
                    FreeVec(iconArray->array[i].icon_text);
                } /* if */
                if (iconArray->array[i].icon_full_path != 0L)
                {
...
IconArray *CreateIconArray(void)
{
    IconArray *iconArray = (IconArray *)AllocVec(sizeof(IconArray), (1UL<<16));
    if (iconArray != 0L)
    {
        iconArray->array = 0L; /* Start with no allocated array */
        iconArray->size = 0;
...
BOOL AddIconToArray(IconArray *iconArray, const FullIconDetails *newIcon)
{
    ...
    if (iconArray->size >= iconArray->capacity)
    {
        ...
        newArray = (FullIconDetails *)AllocVec(newCapacity * sizeof(FullIconDetails), (1UL<<16));
        if (newArray == 0L)
        {
            return 0;
        } /* if */
...
IconArray *CreateIconArrayFromPath(BPTR lock, const char *dirPath)
{
    ...
    iconArray->hasOnlyBorderlessIcons = 1;

    if (!iconArray)
    {
        fprintf((&__iob[2]), "Error: Failed to create icon array.\n");
        return 0L;
    }

    if (!(fib = (struct FileInfoBlock *)AllocDosObject(2, 0L)))
    {
        fprintf((&__iob[2]), "Error: Failed to allocate FileInfoBlock.\n");
    ...
    if (Examine(lock, fib))
    {
        while (ExNext(lock, fib))
        {
            ...
            {
                if (__builtin_strcmp(fib->fib_FileName,"Disk.info") != 0)
                {
                    const char *fileExtension = strrchr(fib->fib_FileName, '.');

                    if (fileExtension != 0L && strncasecmp_custom(fileExtension, ".info", 5) == 0)
                    {
                        ...
                        {
                            iconArray->hasOnlyBorderlessIcons = 0;
                        ...
                        newIcon.icon_max_width = ((iconSize.width) > (textExtent.te_Width) ? (iconSize.width) : (textExtent.te_Width));
                        newIcon.icon_max_height = iconSize.height + 2 + textExtent.te_Height;
                        newIcon.icon_x = iconPosition.x;
                        newIcon.icon_y = iconPosition.y;

                        // Determine if it's a folder or a file
                        newIcon.is_folder = (fib->fib_DirEntryType > 0) ? 1 : 0;

                        // Allocate and copy the full path and icon text
                        newIcon.icon_full_path = (char *)AllocVec(__builtin_strlen(fullPathAndFile) + 1, (1UL<<16));
                        if (!newIcon.icon_full_path)
                        {
                            fprintf((&__iob[2]), "Error: Failed to allocate memory for icon full path.\n");
                            FreeIconArray(iconArray);
                            FreeDosObject(2, fib);
                            return 0L;
                        }
                        __builtin_strcpy(newIcon.icon_full_path,fullPathAndFile);

                        textLength = __builtin_strlen(fib->fib_FileName) + 1;
                        newIcon.icon_text = (char *)AllocVec(textLength, (1UL<<16));
                        if (!newIcon.icon_text)
                        {
                            fprintf((&__iob[2]), "Error: Failed to allocate memory for icon text.\n");
                            FreeVec(newIcon.icon_full_path);
                            FreeIconArray(iconArray);
                            FreeDosObject(2, fib);
                            return 0L;
                        }
                        __builtin_strcpy(newIcon.icon_text,fib->fib_FileName);
                        ...
                        {
                            fprintf((&__iob[2]), "Error: Failed to add icon to array.\n");
    ...
    __builtin_printf("Has only borderless icons: %d\n", iconArray->hasOnlyBorderlessIcons);
    return iconArray;
}
...
void GetNewIconSizePath(const char *filePath, IconSize *newIconSize)
{
    ...
    {
        __builtin_printf("Invalid filePath or newIconSize pointer.\n");
        return;
    }

    // Make a copy of the file path to work with
    filePathLen = __builtin_strlen(filePath);
    filePathCopy = (char *)AllocVec(filePathLen + 1, (1UL<<16));
    if (!filePathCopy)
    {
        __builtin_printf("Memory allocation failed.\n");
        return;
    }
    ...
    if (filePathLen > 5 && __builtin_strcmp(filePathCopy + filePathLen - 5,".info") == 0)
    {
    ...
    if (!diskObject)
    {
        __builtin_printf("Failed to get DiskObject for the file: %s\n", filePathCopy);
    ...
    if (!toolTypes)
    {
        __builtin_printf("Invalid or missing ToolTypes in DiskObject.\n");
    ...
    {
        if (strncmp(toolType, prefix, __builtin_strlen(prefix)) == 0)
        {
            if (__builtin_strlen(toolType) >= 7)
            {
...
BOOL GetStandardIconSize(const char *filePath, IconSize *iconSize)
{
    ...
    filePathLength = __builtin_strlen(filePathCopy);

    /* Remove the ".info" suffix if present */
    if (filePathLength > 5 && __builtin_strcmp(filePathCopy + filePathLength - 5,".info") == 0)
    {
    ...
    if (diskObject == 0L)
    {
        return 0; /* Failed to get the DiskObject */
    }
    ...
    return 1; /* Successfully retrieved and stored the icon size */
}
...
IconPosition GetIconPositionFromPath(const char *iconPath)
{
    ...
    pathLength = __builtin_strlen(iconPath);

    /* Allocate memory for the path copy */
    pathCopy = (char *)AllocVec(pathLength + 1, (1UL<<16));
    if (pathCopy == 0L)
    {
        __builtin_printf("Failed to allocate memory for path copy.\n");
        return position;
    }
    ...
    if (diskObject == 0L)
    {
        __builtin_printf("Failed to load .info file: %s.info\n", pathCopy);
...
BOOL IsNewIconPath(const STRPTR filePath)
{
    BOOL newIconFormat = 0;
    struct DiskObject *diskObject = 0L;
    STRPTR adjustedFilePath = 0L;
    STRPTR *toolTypes;

    /* Check if the provided filepath ends with ".info" */
    size_t len = __builtin_strlen(filePath);
    if (len >= 5 && __builtin_strcmp(filePath + len - 5,".info") == 0)
    {
        /* Allocate memory for the new path without ".info" */
        adjustedFilePath = (STRPTR)AllocVec(len - 4, (1UL<<16));
        if (adjustedFilePath == 0L)
        {
    ...
    else
    {
        ...
        adjustedFilePath = (STRPTR)AllocVec(len + 1, (1UL<<16));
        if (adjustedFilePath == 0L)
        {
            // Memory allocation failed
            return 0;
    ...
    if (toolTypes != 0L)
    {
        while (*toolTypes != 0L)
        {
            ...
            if (__builtin_strcmp(*toolTypes,"*** DON'T EDIT THE FOLLOWING LINES!! ***") == 0)
            {
                newIconFormat = 1;
                __builtin_printf("New icon format detected.\n");
                break;
            }
...
int isOS35IconFormat(const char *filename)
{
    ...
    {
        for (i = 0; i < bytesRead - 12; i++)
        {
            /* Check for "FORM" header */
            if (!foundForm && __builtin_memcmp(buffer + i,"FORM",4) == 0)
            {
            ...
            if (foundForm && __builtin_memcmp(buffer + i + 8,"ICON",4) == 0)
            {
...
int getOS35IconSize(const char *filename, IconSize *size)
{
    ...
    while ((bytesRead = fread(buffer, 1, 1024, file)) > 0)
    {
        ...
        for (i = 0; i < bytesRead - 12; i++)
        {
            ...
            if (foundIcon && __builtin_memcmp(buffer + i,"FACE",4) == 0)
            {
...
int CompareByFolderAndName(const void *a, const void *b)
{
    ...
    return strncasecmp_custom(iconA->icon_text, iconB->icon_text, __builtin_strlen(iconA->icon_text));
}
...
int ArrangeIcons(BPTR lock, char *dirPath, int newWidth)
{
    ...
    __builtin_printf("ArrangeIcons called with path: %s, newWidth: %d\n", dirPath, newWidth);

    iconArray = CreateIconArrayFromPath(lock, dirPath);

    if (iconArray == 0L || iconArray->array == 0L || iconArray->size <= 0)
    {
        return -1;
    }

    totalIcons = iconArray->size;
    __builtin_printf("Total icons: %d\n", totalIcons);
    ...
    __builtin_printf("Largest icon width: %d\n", largestIconWidth);

    // Ensure the largest icon width is reasonable
    if (largestIconWidth <= 0)
    {
        fprintf((&__iob[2]), "Error: Invalid icon width.\n");
        FreeIconArray(iconArray);
        return -1;
    }

    // Calculate the maximum allowable window width based on screen width and preferences
    maxWindowWidth = screenWidth - prefsIControl.currentBarWidth - prefsIControl.currentLeftBarWidth - (4 * 2);
    ...
    {
        maxWindowWidth = screenWidth - prefsIControl.currentBarWidth - prefsIControl.currentLeftBarWidth - (4 * 2);
        __builtin_printf("Adjusted max window width to fit screen: %d\n", maxWindowWidth);
    }
    else
    {
        __builtin_printf("Calculated max window width: %d\n", maxWindowWidth);
    }

    // Determine the optimal number of icons per row
    iconsPerRow = (((newWidth - 10) / (largestIconWidth + iconSpacingX)) > (minIconsPerRow) ? ((newWidth - 10) / (largestIconWidth + iconSpacingX)) : (minIconsPerRow));
    __builtin_printf("Initial icons per row calculated: %d\n", iconsPerRow);

    // Adjust window width to fit these icons evenly
    windowWidth = 10 + iconsPerRow * (largestIconWidth + iconSpacingX);
    __builtin_printf("Adjusted initial window width: %d\n", windowWidth);
    ...
    while (windowWidth < maxWindowWidth && (totalIcons / iconsPerRow) > (iconsPerRow / 2))
    {
        iconsPerRow++;
        windowWidth = 10 + iconsPerRow * (largestIconWidth + iconSpacingX);
        __builtin_printf("Expanding window width: %d, Icons per row: %d\n", windowWidth, iconsPerRow);
    }
    ...
    if (windowWidth > maxWindowWidth)
    {
        ...
        iconsPerRow = (((maxWindowWidth - 10) / (largestIconWidth + iconSpacingX)) > (minIconsPerRow) ? ((maxWindowWidth - 10) / (largestIconWidth + iconSpacingX)) : (minIconsPerRow));
        __builtin_printf("Adjusted to max window width: %d, Icons per row: %d\n", windowWidth, iconsPerRow);
    }
    ...
    for (i = 0; i < totalIcons; i++)
    {
        ...
        columnWidths[column] = ((columnWidths[column]) > (iconArray->array[i].icon_max_width) ? (columnWidths[column]) : (iconArray->array[i].icon_max_width));
    }
    ...
    while (rowStartIndex < totalIcons)
    {
        ...
        {
            maxRowHeight = ((maxRowHeight) > (iconArray->array[i].icon_max_height) ? (maxRowHeight) : (iconArray->array[i].icon_max_height)) + borderSpacingForIconsWithNoSpacing;
        }

        __builtin_printf("Row %d max height: %d\n", rowCount, maxRowHeight);
        ...
        for (i = rowStartIndex; i < rowStartIndex + iconsPerRow && i < totalIcons; i++)
        {
            ...
            __builtin_printf("Placing icon %d at (x: %ld, y: %ld) with width %d and height %d name: %s\n",
            ...
            maxX = ((maxX) > ((iconArray->array[i].icon_x + iconArray->array[i].icon_max_width) + borderSpacingForIconsWithNoSpacing) ? (maxX) : ((iconArray->array[i].icon_x + iconArray->array[i].icon_max_width) + borderSpacingForIconsWithNoSpacing));
            maxY = ((maxY) > ((iconArray->array[i].icon_y + iconArray->array[i].icon_max_height) + borderSpacingForIconsWithNoSpacing) ? (maxY) : ((iconArray->array[i].icon_y + iconArray->array[i].icon_max_height) + borderSpacingForIconsWithNoSpacing));
            __builtin_printf("Updated maxX: %ld, maxY: %ld\n", maxX, maxY);
        ...
        rowCount++;
        __builtin_printf("Moving to next row: y = %ld, rowStartIndex = %d, rowCount = %d\n", y, rowStartIndex, rowCount);
    }

    // Calculate dynamic padding
    dynamicPaddingY = ((10) > (10 / 2) ? (10) : (10 / 2)); // Ensure at least 10 pixels padding

    // Adjust maxY to avoid excessive gap at the bottom
    maxY += dynamicPaddingY; // Add dynamic padding at the bottom
    __builtin_printf("Final adjusted maxY with padding: %ld\n", maxY);

    // Reposition the window to accommodate all icons neatly
    __builtin_printf("Final maxX: %ld, maxY: %ld\n", maxX, maxY);
    ...
    __builtin_printf("!! Done. Icons arranged in %d rows with %d icons per row. Path: %s\n",
